...
namespace Grid
{
	IGrid::IGrid(EGridType gridType,
		...
		, m_type(gridType)
	{
		m_cellSize = sf::Vector2u(m_offset, m_offset);
		update();
	}

	void IGrid::update()
	{
		m_linesAmount = sf::Vector2u(m_size.x/m_offset,  // horizontal lines
									 m_size.y/m_offset); // vertical lines
		m_cellsAmount = sf::Vector2u(m_linesAmount.x-1, m_linesAmount.y-1);

        // create lines
		while (m_linesAmount.y > m_horizontalLines.size())
		{
		...
		while (m_linesAmount.x > m_verticalLines.size())
		{
